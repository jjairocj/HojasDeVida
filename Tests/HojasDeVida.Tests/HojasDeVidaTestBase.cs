...
using HojasDeVida.Migrations.SeedData;
...
namespace HojasDeVida.Tests
{
    public abstract class HojasDeVidaTestBase : AbpIntegratedTestBase<HojasDeVidaTestModule>
    {
        ...
        protected void UsingDbContext(Action<HojasDeVidaDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<HojasDeVidaDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<HojasDeVidaDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<HojasDeVidaDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<HojasDeVidaDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<HojasDeVidaDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<HojasDeVidaDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<HojasDeVidaDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<HojasDeVidaDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<HojasDeVidaDbContext, Task<T>> func)
        {
            T result;
